...
# Guard only UI source; canonical token/config files may hold raw values.
if not re.search(r"/src/.*\.(tsx?|css)$", path): sys.exit(0)
# tokens.ts / tokens.css are the §1 source of truth; tailwind.config maps them.
if re.search(r"(tokens\.(ts|css)|tailwind\.config)", path): sys.exit(0)
v = []
